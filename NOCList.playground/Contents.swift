...
func compromisedAgents(agents: [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]) -> Int {
    ...
    return numOfCompromised
}

//: ## Step 4
//: Call the above function to find the total number of compromised agents and then print a sentence that says "# agents have been compromised!" using string interpolation.
let compTotal = compromisedAgents(agents: agents)
print("\(compTotal) agents have been compromised!")
...
let cleanNum = findCleanAgents(agents: agents)
print("\(cleanNum.count) clean agents out of \(agents.count) total agents")
...
func findHighRisk(agents: [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]) -> [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)] {
    var highRisk: [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)] = []
    
    for agent in agents {
        if agent.accessLevel >= 8 {
            if agent.compromised == true {
                print("\(agent.realName), level: \(agent.accessLevel) **WARNING** **COMPROMISED**")
                highRisk.append(agent)
            } else {
...
func findLevels(agents: [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]) {
    var low = 0
    var med = 0
    var high = 0
    ...
    print("\(low) low level agents, \(med) mid level agents, \(high) high level agents")
}


//: ## Step 10
//: Call the above function and check its output in the console.
findLevels(agents: agents)
